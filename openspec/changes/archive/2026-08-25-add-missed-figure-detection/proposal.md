...
Step 4A only ever sees the figures Azure Document Intelligence reports, so a figure ADI never detects is invisible to the entire pipeline — it is not rejected, not counted, and not indexed. On the confirmed case (a 16-page clinical reference, 16 pages), ADI reported 9 figures and none on pages 6 or 8, while both pages carry real embedded photographs covering roughly 64% and 44% of the page. `step4a-result.json` recorded `rejected: 0`, because there was never a candidate to reject.

Subsequent measurement shows this is not an isolated case. On a 72-page technique guide (72 pages), **45 embedded image placements across 24 of the 72 pages overlap no ADI figure polygon at all** — a third of the document's pages carry at least one image the pipeline cannot see. One confirmed instance is a surgical illustration on page 17 bearing a 30°–50° pin-angle callout: clinically substantive artwork that produces no candidate, no bounding box, and no chunk. The scale of the gap on a mainstream surgical technique guide, rather than a single outlier document, is what establishes this as a systematic recall defect rather than an ADI edge case.
